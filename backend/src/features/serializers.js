export function enrichProject(project, data) {
  ...
  const segment = data.segments?.find((x) => x.id === project.segmentId);
  ...
  const stages = data.projectStages
    ...
    .map((stage) => {
      ...
      return {
        ...
        progressUpdates: (data.taskProgress || []).filter((item) => item.projectStageId === stage.id).map((item) => ({ ...item, byUser: data.users.find((user) => user.id === item.byUserId)?.name || 'Usuario' })).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
        progressPercent: stage.status === 'Completa' ? 100 : Number(stage.progressPercent || 0),
  ...
  return {
    ...
    marketingTaskTemplates: (data.marketingTaskTemplates || []).filter((item) => item.active !== false),
    launchMilestones: (data.launchMilestones || []).filter((item) => item.projectId === project.id).sort((a, b) => new Date(a.date) - new Date(b.date)),
    marketingTasks: (data.marketingTasks || []).filter((item) => item.projectId === project.id).sort((a, b) => new Date(a.startDate || a.dueDate) - new Date(b.startDate || b.dueDate)).map((item) => {
      const template = (data.marketingTaskTemplates || []).find((entry) => entry.id === item.templateId) || null;
      return {
      ...item,
      template,
      area: item.area || template?.area || item.channel || 'Marketing',
      durationDays: Number(item.durationDays || template?.durationDays || 1),
      owner: data.users.find((user) => user.id === item.ownerUserId) || null,
      progressPercent: item.status === 'Completo' ? 100 : Number(item.progressPercent || 0),
      form: template?.form || {
        name: item.title,
        description: 'Plantilla de ejecución para tareas de Marketing.',
        fields: [
          { key: 'objetivo', label: 'Objetivo de la tarea', type: 'textarea', required: true },
      ...
      documents: (data.documents || []).filter((document) => document.marketingTaskId === item.id).map((document) => enrichDocument(document, data)),
      progressUpdates: (data.taskProgress || []).filter((progress) => progress.marketingTaskId === item.id).map((progress) => ({ ...progress, byUser: data.users.find((user) => user.id === progress.byUserId)?.name || 'Usuario' })).sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
      approvals: (data.approvalRequests || []).filter((approval) => approval.marketingTaskId === item.id),
      pendingApproval: (data.approvalRequests || []).find((approval) => approval.marketingTaskId === item.id && approval.status === 'Pendiente') || null
    };
    }),
    stages,
...
export function bootstrapFor(data, user) {
  ...
  const allowedProjects = data.projects.filter((project) => {
    ...
    if ((data.marketingTasks || []).some((task) => task.projectId === project.id && task.ownerUserId === user.id)) return true;
    if ((data.projectBriefs || []).some((brief) => brief.projectId === project.id && brief.marketingReviewerUserId === user.id)) return true;
    if ((data.approvalRequests || []).some((approval) => approval.projectId === project.id && approval.approverUserId === user.id)) return true;
    return data.projectStages.some((stage) => stage.projectId === project.id && (stage.assignedUserId ? stage.assignedUserId === user.id : stage.responsibleRoleId === user.roleId));
  });
  ...
  return {
    ...
    notifications: data.notifications.filter((n) => !n.read && (!n.projectId || allowedProjectIds.has(n.projectId)) && (admin || (n.userId ? n.userId === user.id : !n.roleId || n.roleId === user.roleId))),
    notificationCenter: data.notifications.filter((n) => (!n.projectId || allowedProjectIds.has(n.projectId)) && (admin || (n.userId ? n.userId === user.id : !n.roleId || n.roleId === user.roleId))).slice().sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)),
